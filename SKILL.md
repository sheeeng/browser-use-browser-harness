...
- **If `restart_daemon()` also hangs**, kill Chrome entirely (`pkill -9 -f "Google Chrome"` on POSIX; `taskkill /IM chrome.exe /F` on Windows), clean the daemon endpoint (`from ipc import cleanup_endpoint, pid_path; cleanup_endpoint("default"); pid_path("default").unlink(missing_ok=True)`), reopen Chrome, wait 5s, then reconnect. This resets all CDP state.
